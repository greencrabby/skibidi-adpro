...
# ---Reflection 5---
Reviewing the TDD workflow using the reflective questions proposed by Percival, it is essential to assess whether the process effectively achieves its goals. If the TDD approach is not sufficiently useful, identifying areas for improvement becomes necessary. Based on my experience working on Module 4, I find the TDD workflow to be quite beneficial. By following Percival’s framework, I evaluated my approach to TDD in several aspects. 
In terms of functionality, the tests I have written adequately cover the necessary aspects of the code, ensuring that all critical components are accounted for. So far, the testing process has helped validate the implementation effectively. Additionally, the efficiency of TDD plays a crucial role in the development cycle. While writing tests may initially slow down progress, it ultimately enhances code quality by detecting bugs early. This early detection streamlines the debugging process, making it easier to resolve issues before they become more complex.
Another important factor is the readability and maintainability of the tests. The tests I have created so far are not overly complex, making them easy to understand and eliminating the need for immediate refactoring. This ensures that the testing process remains manageable while still fulfilling its purpose. Overall, the TDD workflow has been valuable in improving both the reliability and efficiency of the development process.

For the F.I.R.S.T principles here are the list along with explanations on how it has been successfully followed:

- Fast: The tests run quickly and do not hinder the development process. So far, they execute efficiently without causing delays.

- Isolated/Independent: Each test is independent and does not rely on others. Dependencies between tests can lead to poor code quality and difficult-to-fix bugs. Currently, the tests remain well-isolated.

- Repeatable: The tests can be consistently executed across different environments. Inconsistencies usually arise when a program depends on external factors such as third-party packages. So far, the packages are well-managed and do not introduce external dependencies, ensuring consistent test results regardless of the environment.

- Self-validating: The tests automatically determine their success or failure. Every necessary test has been properly written to validate expected outcomes.

- Timely: The tests are written at the right time, ideally before the actual code. Following the TDD process, all tests were created before implementing the program logic.